    ...
    <button v-if="canManageDashboard" class="text-[10px] text-gray-400 hover:text-[#F5C518] uppercase tracking-widest font-bold">
      <i class="fa-solid fa-eye mr-1"></i> {{ getAckCount(noti.id) }} Views
            ...
            <section>
              ...
              <div v-for="event in dashboardEvents" :key="event.id" class="bg-[#111111] p-4 md:p-5 rounded-2xl border border-white/5 flex items-center gap-4 md:gap-5 mb-4 relative group">
                ...
                <div><h4 class="font-bold text-white text-sm md:text-base break-words">{{ event.title }}</h4><p class="text-[9px] md:text-[11px] text-gray-500 uppercase tracking-wider mt-1">{{ event.time_str }} | {{ event.location }}</p></div>
              </div>
