              ...
              <tr v-for="user in roster" :key="user.id" class="hover:bg-white/[0.02] transition-colors">
                ...
                <td v-if="['admin', 'president', 'vp'].includes(currentUser?.role)" class="px-4 md:px-8 py-4 md:py-6 text-right pr-6 md:pr-12">
                  <div class="flex justify-end gap-2">
                    <button v-if="currentUser?.role?.toLowerCase() === 'admin' || user.role?.toLowerCase() !== 'admin'" @click="openEditModal(user)" class="text-[10px] font-bold bg-white/5 px-3 py-2 md:px-4 rounded-xl uppercase hover:bg-[#F5C518] hover:text-black transition-all border border-white/5 min-h-[44px]">Edit</button>
                    <button v-if="user.id !== currentUser.id && (currentUser?.role?.toLowerCase() === 'admin' || user.role?.toLowerCase() !== 'admin')" @click="deleteUser(user.id)" class="text-[10px] font-bold bg-white/5 px-3 py-2 md:px-4 rounded-xl uppercase hover:bg-red-500 hover:text-white transition-all border border-white/5 min-h-[44px]"><i class="fa-solid fa-trash"></i></button>
                  </div>
                </td>
              </tr>
            </tbody>
